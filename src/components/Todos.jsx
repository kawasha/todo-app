const Todo = ({ todo, toggleDone, number, deleteTodo, todoUp, todoDown, todosCount, refs, isAdding }) => {

  return (
    <li ref={(el) => (refs.current[number - 1] = el)} className={`flex items-center border border-white p-2 mb-4 ${isAdding ? 'fade-in' : ''}`}>
...
const Todos = ({ todos, toggleDone, deleteTodo, todoUp, todoDown, refs, isAdding }) => {
  return (
      ...
      <ul>
        {
          ...
          todos.map((todo, index) => {
            if (index === todos.length - 1) {
              return <Todo key={todo.id} todo={todo} toggleDone={toggleDone} number={index + 1} deleteTodo={deleteTodo} todoUp={todoUp} todoDown={todoDown} todosCount={todos.length} refs={refs} isAdding={isAdding} />
            }
            return <Todo key={todo.id} todo={todo} toggleDone={toggleDone} number={index + 1} deleteTodo={deleteTodo} todoUp={todoUp} todoDown={todoDown} todosCount={todos.length} refs={refs} />
          })
          :
          todos.map((todo, index) => {
            return <Todo key={todo.id} todo={todo} toggleDone={toggleDone} number={index + 1} deleteTodo={deleteTodo} todoUp={todoUp} todoDown={todoDown} todosCount={todos.length} refs={refs} />
